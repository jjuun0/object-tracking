...
def define_tracker():
    ...
    tracker_types = ['BOOSTING', 'MIL', 'KCF', 'TLD', 'MEDIANFLOW', 'GOTURN', 'MOSSE', 'CSRT']  # GOTURN : error
...
def tracking(tracker_type, tracker, image_folder):
...
if __name__ == '__main__':
    ...
    tracking(my_tracker_type, my_tracker, my_image_folder)

    # for new_frame in range(3):
    #
    #     numbering = str(new_frame).zfill(3)  # 000, 001, 002, ,,,
    #     new_frame_folder = image_folder + numbering + '/focal/'
    #
    #     for focal_num in range(100):
    #         numbering = str(focal_num).zfill(3)
    #         new_image_address = new_frame_folder + numbering + '.png'
    #
    #         # Read a new frame
    #         new_image = cv2.imread(new_image_address)
    #
    #         # Update tracker
    #         ok, bbox = my_tracker.update(new_image)
    #
    #         # Draw bounding box
    #         if ok:
    #             # Tracking success
    ...
    #
    #         # Display tracker type on frame
    #         cv2.putText(new_image, my_tracker_type + " Tracker", (130, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
    #                     (50, 170, 50), 2)
    #
    #         # Display result
    #         cv2.imshow("Tracking", new_image)
